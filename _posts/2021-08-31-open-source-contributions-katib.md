...
본 포스팅에서는 kubeflow 의 구성요소 중 **Hyperparameter Tuning and Neural Architecture Search** 기능을 제공하는 [katib](https://github.com/kubeflow/katib) 를 사용하며 겪었던 불편 사항과 해당 기능을 구현하여 오픈소스 프로젝트에 기여한 경험을 위주로 공유드리려고 합니다. katib 에 대해 보다 자세한 정보가 궁금하시다면 다음 [공식 문서](https://www.kubeflow.org/docs/components/katib/overview/)를 확인해 주시기 바랍니다.
...
따라서 흔히 사용하는 Hpo package 인 [Scikit-Optimize](https://scikit-optimize.github.io/stable/user_guide.html), [Hyperopt](https://github.com/hyperopt/hyperopt), [Optuna](https://optuna.readthedocs.io/en/stable/) 를 로컬 머신의 제한된 스펙에서 수행하는 작업은 한계가 있다고 볼 수 있습니다. 그보다 kubernetes 와 같은 container orchestration system 을 활용해 node 단위의 병렬 작업과 리소스 관리 최적화 및 스케줄링을 자동화한다면 훨씬 효율적인 실험을 할 수 있습니다.

물론 Optuna 와 Ray-tune 을 비롯한 Hpo package 들에서도, 외부 서버 cluster 혹은 kubernetes cluster 에서 worker 를 나누어 병렬 작업을 수행할 수 있는 기능을 지원하고 있습니다. 하지만 이들은 처음부터 kubernetes native 하게 설계되지 않은 프로젝트이기 때문에 kubernetes 와 통합하기에는 다소 사용성이 떨어지는 부분이 존재합니다.

katib 는 이러한 문제를 효과적으로 해결하기 위해 시작된 프로젝트입니다.
katib 는 자체적으로 hyperparameter search algorithm 을 구현하기도 하지만, 가장 큰 특징은 skopt, hyperopt, chocolate, optuna 와 같은 외부 라이브러리들의 기존 method 들을 **쉽게 통합할 수 있는 인터페이스를 제공**한다는 점입니다. 인터페이스에 맞게 통합할 때 필요한 [가이드](https://github.com/kubeflow/katib/blob/master/docs/new-algorithm-service.md)도 자세히 제공하고 있으며, 실제로 이에 맞게 skopt, chocolate, optuna 등의 구현체가 katib 에 [통합되어 있습니다](https://github.com/kubeflow/katib/tree/master/pkg/suggestion/v1beta1).
즉, katib 는 hyperparameter search algorithm 의 구현체들을 모아둔 프로젝트라기보다는, 구현체들이 kubernetes 내의 자원들을 효율적으로 사용하며 Hpo 를 수행할 수 있게 api service 화를 담당하고, 그 서비스를 안정적으로 제공할 수 있는 인프라를 제공하는 프로젝트에 가깝다고 표현할 수 있습니다.
...
katib 는 Hpo 한 세트를 `Experiment` 라는 [custom resource](https://kubernetes.io/ko/docs/concepts/extend-kubernetes/api-extension/custom-resources/) 로 정의하고 관리하고 있습니다.
...
또한, 2 차적으로는 katib 에서도 비정상적인 입력을 좀 더 정교하게 처리하기 위해서 해당 이슈를 재현할 수 있는 명령과 스크린샷을 자세하게 첨부한 issue 를 생성한 후, 해당 기능을 추가한 코드와 테스트 코드를 추가한 **PR** 을 진행하였습니다.

이렇게 머지된 PR 중 두 가지 PR 에 대한 내용을 간략하게 소개 드리면 다음과 같습니다.
...
<figure class="image" style="align: center;">
<p align="center">
  <img src="/assets/images/2021-08-31-open-source-contributions-katib/1-code.png" alt="" width="80%">
...
[두 번째](https://github.com/kubeflow/katib/pull/1600)로는 `Experiment` 에서 Hyperparameter Search Algorithm 을 선택할 때, 해당 algorithm 의 세팅을 잘못 입력한 경우에 대한 Validation 로직이 없는 algorithm 이 일부 존재하여, 생성 요청한 experiment 가 `RUNNING` 으로 영원히 멈춰있는 이슈였습니다.
katib 에서는 해당 [issue](https://github.com/kubeflow/katib/issues/1126) 를 이미 파악하고 있었지만, 약 1 년째 진행되지 않고 open & frozen 상태로 남아있는 상황이었습니다.
저희 마키나락스에서는 bayesian optimization algorithm 으로 hyperparameter search 를 수행하는 실험을 진행 중 동일한 문제 상황을 겪었기에, 프로젝트에 직접 기여하기로 하였습니다.
먼저 katib 프로젝트의 해당 이슈에 이슈 상황에 대해 조금 더 자세히 알려달라는 코멘트를 남겼고, katib 의 Maintainer 중 한 명인 [@andreyvelich](https://github.com/andreyvelich) 가 친절하고 빠르게 답변해주어 소스코드 상의 어디가 문제인지, 어떻게 고쳐야 하는지에 대해 보다 빠르게 파악할 수 있었습니다.
...
katib 에서 제공하는 hyperparameter search algorithm 중 Hyperopt 는 [다음](https://github.com/kubeflow/katib/blob/3fadef637ad17458f629a4baeba7fd38205a1510/pkg/suggestion/v1beta1/hyperopt/service.py#L57)과 같이 algorithm 의 Validation 과정을 제공하고 있었지만, skopt 를 비롯한 일부 algorithm 은 Validation 과정을 제공하고 있지 않는다는 문제가 있었습니다.

따라서, skopt 의 optimizer 의 공식 api [문서](https://scikit-optimize.github.io/stable/modules/generated/skopt.Optimizer.html)를 따라, skopt 에서 필요한 validation 이 무엇인지 확인한 후, 그에 맞게 validation 하는 로직을 추가하고, 각각의 경우에 대한 테스트케이스를 검증하는 test code 를 추가한 PR 을 작성하게 되었습니다.

<figure class="image" style="align: center;">
<p align="center">
  <img src="/assets/images/2021-08-31-open-source-contributions-katib/skopt-validation-수정.png" alt="" width="80%">
...
위의 PR 외에도 kubeflow/katib 를 비롯해 [kubeflow/kubeflow](https://github.com/kubeflow/kubeflow), [kubeflow/pipeline](https://github.com/kubeflow/pipelines) 등 마키나락스에서 활용하며 만났던 이슈들과 그 해결책들을 지속적으로 제시하다 보니, 기존 kubeflow member 의 추천을 받아 kubeflow organization 의 member 로 [합류](https://github.com/kubeflow/internal-acls/pull/487)하게 되었습니다. 
...
저희 마키나락스에서는 kubeflow 외에도 다양한 오픈소스들을 적극적으로 활용하고 있으며, 단순한 사용자에 그치기보다는 오픈소스로부터 받았던 도움을 다시 커뮤니티에 돌려주는 컨트리뷰터가 되기를 장려하고 있습니다. 저 또한 kubeflow member 로써 앞으로 더 적극적인 기여를 지속하는 것을 목표로 하고 있습니다.
